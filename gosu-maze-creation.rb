...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    col_index = 0
    # the following code loops thru the entire grid and sets up neighbor links for all the cells
    # the code will skips over the border cells who will not have neighbors
    while(col_index < x_cell_count)
      row_index = 0

      # skips border cells on the sides so that there arent neighbor links added to them
      while(row_index < y_cell_count)
        if(col_index != 0)
          @columns[col_index][row_index].north = @columns[col_index-1][row_index]
        end
        if(col_index != 9)
          @columns[col_index][row_index].south = @columns[col_index+1][row_index]
        end
        if(row_index != 9)
          @columns[col_index][row_index].east = @columns[col_index][row_index+1]
        end
        if(row_index != 0)
          @columns[col_index][row_index].west = @columns[col_index][row_index-1]
        end
        row_index += 1
      end
      col_index += 1
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
     ...
     # checks neighbouring cells to see if they are vacant and have not already been visited by the recursive search
      if (@columns[cell_x][cell_y-1] != nil && (@columns[cell_x][cell_y-1].vacant) && !(@columns[cell_x][cell_y-1].visited))
        north_path = [cell_x, cell_y-1]
      elsif (@columns[cell_x][cell_y+1] != nil && (@columns[cell_x][cell_y+1].vacant) && !(@columns[cell_x][cell_y+1].visited))
        south_path = [cell_x, cell_y+1]
      elsif (@columns[cell_x+1][cell_y] != nil && @columns[cell_x+1][cell_y].vacant && !@columns[cell_x+1][cell_y].visited)
        east_path = [cell_x+1, cell_y]
      elsif (@columns[cell_x-1][cell_y] != nil && (@columns[cell_x-1][cell_y].vacant) && !@columns[cell_x-1][cell_y].visited)
        west_path = [cell_x-1, cell_y]
      end

      # marks the current cell as visited so it cant be visited again
      ...
      if (path != nil)
        ...
        search(path[0], path[1]).push([cell_x, cell_y]) # recursive call
      else
        if (ARGV.length > 0) # debug
